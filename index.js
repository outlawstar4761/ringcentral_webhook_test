...
const emailData = {
  to:['jjwatson@ensafe.com','jlivingston@ensafe.com'],
...
function _buildEmailBody(data){
  return `
  <p>This is an example email generated by John's existing messenger service. I wanted to <ul><li>show off</li><li>prove sending an email without putting in all the leg work required to talk to 0365<li><ul><br>I'll be working on that next. Let me know when you have some time and I'll show you all the guts.</p>
  <h4>A Telephony Disconnect Event has occured</h4>
  <b>To</b>
  <pre>${data.parties[0].to.phoneNumber}</pre>
  <b>From</b>
  <pre>${data.parties[0].from.phoneNumber}</pre>
  `;
}
